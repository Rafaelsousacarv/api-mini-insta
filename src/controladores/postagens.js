...
const curtir = async (req, res) => {
  ...
  try {
    ...
    if (jaCurtiu) {
      return res.status(404).json('O usuarios ja curtiu essa postagem');
    }
...
const comentar = async (req, res) => {
  ...
  try {
    ...
    if (!texto) {
      return res
        .status(404)
        .json('Para comentar nesta postagem é necessario informar o texto');
    }
...
const feed = async (req, res) => {
  ...
  try {
    // const postagem = await knex('postagens').limite(10).offset(o);
